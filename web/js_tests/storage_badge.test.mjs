...
import { describeStorageBadge } from '../js/lib/storage_badge.js';

test('sqlite mode with a path → DB-path tooltip', () => {
  const v = describeStorageBadge('sqlite', '/home/user/.config/aceman/favorites.db');
...
test('unknown mode defaults to browser', () => {
  ...
  const v = describeStorageBadge('rogue');
  assert.equal(v.text, 'browser');
});

test('undefined mode is treated as browser too', () => {
  const v = describeStorageBadge();
  assert.equal(v.text, 'browser');
});
