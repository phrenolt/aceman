...
import { findFavByCid } from '../js/lib/fav_lookup.js';
...
test('finds an entry by exact lowercase cid', () => {
  assert.equal(findFavByCid(FAVS, CID_A).name, 'Channel A');
});

test('finds an entry case-insensitively (uppercase query)', () => {
  assert.equal(findFavByCid(FAVS, CID_A.toUpperCase()).name, 'Channel A');
});
...
test('finds an entry case-insensitively (mixed-case stored cid)', () => {
  ...
  assert.equal(findFavByCid(favs, needle).name, 'Mixed');
});

test('returns null when nothing matches', () => {
  assert.equal(findFavByCid(FAVS, 'c'.repeat(40)), null);
});

test('returns null on empty / non-array favs', () => {
  assert.equal(findFavByCid([], CID_A), null);
  assert.equal(findFavByCid(null, CID_A), null);
  assert.equal(findFavByCid(undefined, CID_A), null);
});

test('returns null on empty / non-string cid', () => {
  assert.equal(findFavByCid(FAVS, ''), null);
  assert.equal(findFavByCid(FAVS, null), null);
  assert.equal(findFavByCid(FAVS, 42), null);
});
...
test('skips entries with malformed shape (defensive)', () => {
  ...
  assert.equal(findFavByCid(favs, CID_A).name, 'real');
});
...
test('returns the FIRST matching entry when duplicates exist', () => {
  ...
  assert.equal(findFavByCid(favs, CID_A).name, 'first');
});
