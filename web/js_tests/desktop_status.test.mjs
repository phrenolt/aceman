...
import { describeDesktopStatus } from '../js/lib/desktop_status.js';

test('null input → unavailable card with disabled Install button', () => {
  const v = describeDesktopStatus(null);
...
test('missing path → empty string (no crash, no "undefined" in DOM)', () => {
  const v = describeDesktopStatus({ installed: true });
  assert.equal(v.path, '');
});
...
test('install vs uninstall actions never collide on disabled state', () => {
  ...
  const installed = describeDesktopStatus({ installed: true });
  assert.equal(installed.button.action, 'uninstall');
  assert.equal(installed.button.text, 'Uninstall');

  const notInstalled = describeDesktopStatus({ installed: false });
...
test('explicit installed=false matches not-installed branch', () => {
  ...
  const a = describeDesktopStatus({ installed: false });
  const b = describeDesktopStatus({});
