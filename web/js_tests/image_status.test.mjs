...
import { describeImageStatus } from '../js/lib/image_status.js';

test('null input → unavailable card (e.g. fetch failed)', () => {
  const v = describeImageStatus(null);
...
test('installed with no log → log card hidden', () => {
  const v = describeImageStatus({ state: 'idle', installed: true });
  assert.equal(v.log.visible, false);
});

test('not installed → Install enabled, Uninstall disabled', () => {
  const v = describeImageStatus({ state: 'idle', installed: false });
