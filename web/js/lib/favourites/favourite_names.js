...
//
// `uniqueFavouriteName(label, takenNames)` normalises a candidate label
...
//
// `pickFavouriteName(result, takenNames)` picks the default name for a
...
export function uniqueFavouriteName(label, takenNames) {
...
export function pickFavouriteName(r, takenNames) {
  return uniqueFavouriteName(r.translated_name || r.name, takenNames) ||
         uniqueFavouriteName('ace ' + (r.cid || '').slice(0, 8), takenNames);
}
