...
//
//   import { createBrowserFavs } from './browser_favs.js';
//   const favs = createBrowserFavs();        // production
//   const favs = createBrowserFavs(fakeLS);  // tests
//
...
export function createBrowserFavs(storage) {
  const ls = storage || (typeof localStorage !== 'undefined' ? localStorage : null);
  if (!ls) throw new Error('createBrowserFavs: no storage backend');
